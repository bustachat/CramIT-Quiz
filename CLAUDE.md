...
│   ├── mathematics-standard-2.json    ← 318 MC + 151 written + 73 tips + studyNotes (5 of 16 topics so far — F1 Money Matters, F4 Investments & Loans, M1 Measurement, M7 Rates & Ratios, A4 Non-linear Relationships; no writingScaffolds, being built topic-by-topic, 11 categories remaining, none queued yet)
...
| Study Mode & Exam Mode front page (HMS prototype + Maths in progress) | 🔶 HMS built 2026-07-26 (full — diagrams, comparison tables, 74 revision questions, free-preview gating). Maths started 2026-07-28, built **one topic at a time by owner's explicit preference** (declined batching even after being told it was low-risk — see feedback memory) — 5 of 16 syllabus categories done (F1 Money Matters, F4 Investments & Loans, M1 Measurement, M7 Rates & Ratios, A4 Non-linear Relationships; original-wording notes since Maths has no standalone prose source to port from, unlike HMS), 11 categories remaining, none queued yet, no Notes/Writing Help toggle for Maths at all (hidden when `writingScaffolds` is empty — data-driven, HMS unaffected). Not yet started for Multimedia/VET. HMS's 4 non-photo diagrams (SMART goals grid, tapering bar chart, inverted-U SVG, HOWSCSE grid) still deferred — need bespoke CSS ported, not just an image copy. |
...
*CLAUDE.md — CramIT Project — Last updated: 2026-07-28 — Study Mode Maths build: A4 Non-linear Relationships added, completing the queued M1/M7/A4 batch (5 of 16 topics total, one-at-a-time by owner preference, none queued next), following F4 Investments & Loans and the F1 Money Matters pilot earlier the same day; Notes/Writing Help toggle hidden when a subject has no writingScaffolds (§6/§10/§11); earlier (2026-07-27): an automated billing test harness, a "Manage subjects" removal feature, and deferred-downgrade billing (Stripe Subscription Schedules) were built, broke live, rolled back, and were fixed and confirmed working — see `docs/HISTORY.md`; that session also phased the Study Mode rollout (P1 Maths, P2 VET, then Multimedia — superseded now that Maths work has started) and flagged a design aesthetics review as next focus (§11); before that: Study Mode/Exam Mode front page added to index.html (HMS prototype); before that: Content Agent rebuilt (triage + generation, PR-only, nightly via content-agent.yml); before that: restructured, session history moved to docs/HISTORY.md.*
