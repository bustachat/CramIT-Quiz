...
*CLAUDE.md — CramIT Project — Last updated: 2026-06-21 — All 72 written question stimulus images converted to inline `<img>` tags in q field (top-level image field → null). All diagrams re-cropped to diagram-only bounds at 3x resolution. HTML tables used for z-tables and data tables. `written_q_extracts.json` is the permanent reference file — never re-extract from PDFs. writtenQuestions total: 151. All 318 MC categories verified against NESA MGs — 33 variant codes fixed (F2→F4, M4→M7, M5→M6, N1→N2).*
