...
| Health & Movement Science (HMS) | `hms` | `pdhpe-hms` | MC + written | 193 MC, 40 written | ✅ `CLAUDE.AI - HMS_In_Depth_Study_YR12_quiz.html` |
...
*CLAUDE.md — CramIT Project — Last updated: 2026-08-25 — Two new mandatory rules in §10, both written after real failures this session: (1) **exam citations must be verified against the actual paper**, never a third-party claim — a prior session shipped a question labelled "HSC 2024, PDHPE Exam, Section I, Part B, Q31.b" that was wrong on section, marks and wording, and described it in `docs/HISTORY.md` as "a real sourced HSC 2024 question" without ever opening `NESA Exams Folder/PDHE/2024-hsc-pdhpe.pdf`, which was sitting on disk; (2) **diff a new block against its neighbours before inserting it into an existing Study Mode topic** — an accuracy audit is not an editorial review, and two tables added to fill a "no visual" gap duplicated the prose boxes directly above them until the owner spotted it. Same section records that `validate_subjects.cjs` does **not** existence-check `studyNotes` block images (only question images), so study images need browser verification, and that they are `loading="lazy"` so `naturalWidth` reads 0 in a hidden Browser pane. No credential, schema or file-structure facts changed. HMS `biomechanics-recovery-injury` was audited against the owner’s school workbook and rebuilt (17→19 blocks): three factual fixes, the three `fa2_*` topics refiled from the FA1 to the FA2 picker bucket where the syllabus puts them, one CC BY 4.0 OpenStax fracture-types diagram added to `/diagrams/`, two syllabus concept maps built natively as `table` blocks rather than sourced as images, and a full de-duplication pass. Full detail in `docs/HISTORY.md` (2026-08-25 entries). Earlier (2026-08-08): CI security scanning added: new `.github/workflows/security-scan.yml` (Semgrep + Gitleaks + Trivy), additive alongside `validate.yml`/`content-agent.yml` (neither touched). Runs on push/PR to `main`, weekly, and manual dispatch — Gitleaks blocks the build on a found secret (repo is public, holds a live Stripe integration, and `main` auto-deploys with no staging gate), Semgrep/Trivy are report-only to the Security → Code scanning tab. Action versions confirmed current via `gh api` before writing the workflow, not guessed; pattern replicated from a sibling repo (`bustachat/olivier-guide`) already verified working there. First run (`31254692886`) confirmed green via `gh run watch --exit-status` — all three jobs passed, no fixes needed. Full detail in `docs/HISTORY.md`. Flagged, not fixed: `.gitignore` doesn't list `.env` — out of scope for this task, left for the owner. Earlier (2026-07-30): full VET Construction Study Mode built and shipped in one session (9/9 topics + 27 tool/PPE icons), `main` up to date, working tree clean, validator green. VET Construction's Hand & Power Tools and Safe Work Practices topics gained 27 tool/PPE reference icons (self-hosted in `icons/vet-construction/`, sourced from Flaticon author `juicy_fish`'s free "Flat" style — see §11 known issues for the still-unplaced attribution credit, and `docs/HISTORY.md` for the full sourcing story, including catching a broken Vecteezy preview link and a style-mismatched freesvg.org link the owner had picked before they were wired in). Earlier same day: VET Study Mode complete (§11): all 9 topics built (Safety: WHS & Risk Management + Safe Work Practices & Emergencies, Plans/Drawings/Specifications, Measurements & Calculations, Hand & Power Tools, Tool Selection/Safety/Care/Security, Industry Structure/Careers/Employment, Work Instructions/Teamwork/Workplace Conduct, Task Planning & Sustainability — 71 blocks, 54 revision questions), following the new mandatory syllabus-check rule (§10) established earlier the same day: the topic list was verified against the actual official NESA syllabus (`construction-curriculum-framework-syllabus-cpc-v9.1.docx`, downloaded to `NESA Exams Folder/VET - Construction/`), correcting an earlier mapping-grid-derived 7-topic draft to a syllabus-verified 9 topics proportional to each focus area's real content density — see `docs/HISTORY.md` for the full derivation and per-topic build detail. `hasStudy: true` wired into `index.html` for `vet`. Owner said "continue until complete" after topic 1, a one-off pacing override (same pattern as Multimedia's topics 3–7); pushes were grouped by syllabus focus-area boundaries (4 commits total) rather than literally one per topic. One icon collision (topic 6's 🔒 vs the gating lock badge) was caught during final browser regression and fixed to 🧰. VET now has full Study Mode parity with HMS (9 topics) and Multimedia (7 topics); earlier (2026-07-29): Multimedia Study Mode complete: all 7 topics built (Text & Document Design, Graphics, Animation, Video, Audio, World Wide Web, Intellectual Property & Ethics — 47 blocks, 42 revision questions), topic list grounded in the official NESA marking-guideline mapping grids (all 6 years, 2020–2025) rather than keyword-matched against the exam bank, surfacing a known gap — Section III (15 marks/exam, rotating business/industry themes) was never ported into the bank; owner decided Section III becomes a Multimedia "upgrade phase" sequenced AFTER VET Study Mode is built, so VET is now next in the queue ahead of it (owner starting VET in a fresh session); topics 1–2 built one-at-a-time per the owner's standing pacing preference, then the owner explicitly said "keep going until complete" so topics 3–7 were built in one continuous pass — a one-off override, not a change to the standing preference; Writing Help (single "Short Answer 1–5 marks" scaffold) is a separate, smaller follow-up, not yet built; session closed out clean — `main` up to date, working tree clean, validator green (§6/§11); earlier (2026-07-28): Study Mode Maths build completed and reordered: all 16 syllabus categories now have notes, alphabetically ordered A1→S5 (§6/§10/§11 — note the free-preview topic shifted from F1 to A1 as a result), Notes/Writing Help toggle hidden when a subject has no writingScaffolds; earlier (2026-07-27): an automated billing test harness, a "Manage subjects" removal feature, and deferred-downgrade billing (Stripe Subscription Schedules) were built, broke live, rolled back, and were fixed and confirmed working — see `docs/HISTORY.md`; that session also phased the Study Mode rollout (P1 Maths, P2 VET, then Multimedia — superseded now that Maths work has started) and flagged a design aesthetics review as next focus (§11); before that: Study Mode/Exam Mode front page added to index.html (HMS prototype); before that: Content Agent rebuilt (triage + generation, PR-only, nightly via content-agent.yml); before that: restructured, session history moved to docs/HISTORY.md.*
