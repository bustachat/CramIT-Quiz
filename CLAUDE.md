...
| **Mathematics Advanced — new subject port** | 🔶 **Stages 0 and 2 complete 2026-08-27.** Stage 0 = **GO** (`docs/paper-reports/mathematics-advanced.md`): 2020–2025 papers + guidelines local, 10 MC + 90 written marks/paper, ~93% portable, notation `basic`. Stage 2 complete (`docs/subject-plans/mathematics-advanced.md`): official **2017** syllabus read (14 subtopics, 358 content dot points), `category` codes fixed, and `data/mapping-grid/mathematics-advanced.json` gives every question's official topic. ⚠️ The **2024** syllabus takes over at the **2027 HSC** — this topic list is dated. **Stage 1 (Survey) is the only outstanding stage before Stage 3; nothing is blocked.** Nothing ported; subject registered nowhere in code. Two priced-in costs: ~100 image assets (≈5× VET's load) and a garbled text layer forcing manual transcription of Section II. |
