...
│       │                                with a fingerprint of the official sample answer AS
│       │                                AT REVIEW TIME — so regenerating the key VOIDS a
│       │                                review rather than letting it go stale. Built by
...
| **`build_written_key.py`'s criteria rows can be word-order scrambled** | Found 2026-09-05, by USING them: a row containing an inline symbol or superscript comes out with its tail hoisted to the front — 2025 Standard 2 Q25(b) reads *"y-intercept Provides correct interpretation of both slope and"*, and Advanced 2020 Q13 *"sec2 Finds the anti-derivative of x"*. Measured across the committed keys: **Advanced 42/540 rows, Standard 2 20/510, VET 1/251, Multimedia 0/146** — it tracks mathematical notation, so the maths subjects carry nearly all of it. Same class as the two cell-order bugs already fixed in `build_mapping_grid.py` and this extractor. **Marks and sampleAnswers are unaffected**, so nothing in CI notices. Detected by a heuristic (row starts lowercase, or ends on a dangling `and`/`of`/`or`), which is a smell test, not a parser — the real fix is to order the spans by x within each line, and it needs the local PDFs. Until then a part whose criteria row trips the heuristic gets **no** `bandDescriptors` and falls back to the engine's generic wording (6 parts in Standard 2), so scrambled NESA text is never shown to a student. |
...
| **Per-part answer boxes & per-part marks (multi-part written questions)** | 🔶 **Engine built and live 2026-09-05; data done for VET only.** A written question NESA prints with lettered parts now gets an answer box and a mark PER PART, via a new `parts[]` array on the bank entry (CLAUDE.md §10 rule 10; gate in `docs/porting-playbook.md` Stage 3). UI is an accordion with auto-advance — chosen over straight scroll and step-through after mocking all three at phone size: **2024 Q19 is 1.0 screens this way against 2.1 stacked**, and unlike step-through you can still jump freely between parts, as you would on the real paper. Offline scoring runs each part against its OWN keywords/marks; AI marking sends every part in **ONE** request and returns a mark each, so the student's monthly quota is not multiplied by the part count (server clamps each part to its own max, discards invented labels, derives the total from the parts — 6 tests). `validate_subjects.cjs` asserts `sum(parts[].marks) === marks`. ⚠️ **The live AI call has still never been made** (no `ANTHROPIC_API_KEY` here) — the prompt path is verified from the function's own source, the response path by tests. **Backlog: VET (18 questions) and Mathematics Standard 2 (66) carry `parts[]`.** Maths Advanced (126 written), Multimedia (29) and HMS (40) still have **no per-part keywords** — per-part marks and band descriptors are both recoverable from `data/answer-key/written/`, but keywords are not, and Standard 2's only worked because its own model answers are split by part and its keywords are answer fragments. Those subjects keep today's single box until that content exists. |
