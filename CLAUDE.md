...
8. **Written marks are ground truth too, in `data/answer-key/written/`.** Built by
   ...
   `docs/porting-playbook.md` §6; **none of the repo's 369 written answers is reviewed yet**.
...
| **Written-answer review — 369 questions, none reviewed** | ⬜ **Designed 2026-09-01; first session scheduled as Multimedia Stage 6b.** `check_written_key.cjs` enforces the written **mark only** — prose cannot be compared for equality — so **everything a written question teaches or is marked on is outside CI**: `modelAnswer` (which `index.html` shows **directly to the student**, and which `mark-written.js` is **never sent** — a wrong one is a pure teaching defect nothing reports), `keywords` (drives AI marking *and* the offline fallback grid) and `bandDescriptors` (the AI's band rubric). **Measured coverage 2026-09-01** across 369 written questions: `modelAnswer` 369/369, `keywords` **322/369**, `bandDescriptors` **342/369** — and **0/369 reviewed against NESA's sample answers, ever**. ⚠️ **VET has 0 of 23 `bandDescriptors`**, so every VET written question is AI-marked on a generic 0/50%/100% fallback rubric — the worst cell in the table, and missing data rather than an unreviewed one. ⚠️ **VET is also where the 2026-08-27 MC pass found 6 wrong answers in 75, every one carrying an `optionExplanations` entry arguing for the wrong answer** — same authoring, same period, written prose never read back; **if the backlog is prioritised rather than done in full, VET goes first.** Design (review ledger + sample-answer fingerprint so a regenerated key voids a stale review; report-then-enforce ramp; mechanical triage that orders the reading queue but **never decides**) is in `docs/porting-playbook.md` §6, and is now a Gate 6 and Definition-of-Done item for **every future port**. ⚠️ **`bandDescriptors` have no ground truth**: `build_written_key.py` extracts the mark and sample answer but **not the criteria table**, so reviewing that third field properly needs a scoped extractor extension — worth doing once, not per subject. |
