...
| **VET 2021 Q15's four cross-section option images were never cropped** | The question asks which cross-section matches the site plan's contours, but `/diagrams/vet-construction_2021_Q15_stimulus.jpg` is the site plan **only** — the four answer diagrams (page 7 of `2021-hsc-vet-construction.pdf`) were never extracted. Options are currently accurate *text* descriptions of the four curves (rewritten 2026-08-27; the previous `Flat / Slope up / Slope down / Valley` described the wrong pictures), so the question is answerable and the answer key passes. Cropping the four and wiring them as `optionImages` would render it as the real paper does. **This is the only known question of its kind — worth confirming that while doing it:** re-check every question carrying an `image` whose options are bare letters or short labels, in case another paper hides the same gap. Use `scripts/extract_written_diagrams.py` / the `diagram_registry.json` crop pattern; note `validate_subjects.cjs` existence-checks `optionImages`, so a wrong path fails CI (unlike `studyNotes` images, see §10). |
