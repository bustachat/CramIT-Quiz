...
- **Security pass — function auth + entitlement (2026-07-02)** — All Cloudflare Pages Functions now require a verified Supabase JWT; identity is derived server-side, never from the request body.
  ...
  - `index.html`: new `authHeaders()` helper; all 5 call sites send the bearer token; bodies no longer carry user_id/customer_id/subscription_id/plan_type; `x-user-email` header removed.
  - **⬜ OWNER ACTION — run `migrations/2026-07-02_subject_entitlement.sql` in Supabase SQL Editor.** Adds a BEFORE INSERT trigger on `subject_selections` capping selections at the paid `subject_count` (unlimited=7, flex≥7, no active sub=0) + a unique (user_id, subject_id) index. Until it runs, entitlement is still client-only.
  - **⬜ OWNER ACTION — test on the deployed site with the Stripe sandbox** (functions can't run locally): sign in → subscribe 2 subjects with `4242…` → confirm unlock; Billing opens the portal; AI marking works on a written question; and in the console `fetch('/customer-portal',{method:'POST'})` (no auth header) returns **401**. Timing note: `handlePaymentReturn` inserts selections after polling the webhook (~20s max) — with the trigger live, a slower webhook means inserts fail once and the user re-taps the subject; acceptable pre-launch, revisit if seen in testing.
