...
│   └── vet-construction.json          ← 75 MC + 23 written. ✅ WRITTEN-ANSWER REVIEWED
│                                        2026-09-01 (the only reviewed subject): all 23
│                                        carry `bandDescriptors` derived from NESA's
│                                        criteria and `keywords`/`minKeywords`; 15 ok,
│                                        6 corrected, 2 divergent-accepted. Runbook:
...
8. **Written marks are ground truth too, in `data/answer-key/written/`.** Built by
   ...
   **VET Construction is the only reviewed subject, 23/23; the other 346 of 369 are not.**
...
| **Written-answer review — VET done, 346 of 369 to go** | 🔶 **Mechanism BUILT AND RUNNING 2026-09-01; VET Construction reviewed 23/23, the first subject ever.** Runbook: `docs/subject-plans/vet-construction-written-review.md`. `check_written_key.cjs` enforces the written **mark only** — prose cannot be compared for equality — so **everything a written question teaches or is marked on is outside CI**: `modelAnswer` (which `index.html` shows **directly to the student**, and which `mark-written.js` is **never sent**), `keywords` (drives AI marking *and* the offline fallback grid) and `bandDescriptors` (the AI's band rubric). **The design is now real, not planned**: a review ledger at `data/answer-key/written/reviews/{subject}.json` (built by `scripts/build_review_ledger.py` from a hand-typed verdict table in `scripts/reviews/`), each entry carrying a **fingerprint of NESA's sample answer as at review time** so regenerating the key VOIDS the review rather than letting it go stale; and a report-then-enforce ramp where **committing a ledger opts that subject in to hard CI enforcement** (proved: a corrupted fingerprint exits 1). ⚠️ **`bandDescriptors` now HAVE ground truth** — `build_written_key.py` extracts the criteria rows too (1 446 across the four subjects), so descriptors are NESA's own wording; the 1-to-5-band → `{full, partial, minimal}` collapse rule is recorded in the runbook. **The VET review found 6 real defects in 23, every one invisible to CI because the mark was right** — a model answer with a confidently wrong result (2023 19(b)(i): 2.61 m³ vs NESA's 2.99, the centre beam omitted and the gap explained away), a model answer quoting two fabricated stimulus band labels (2022 19(a)), an `acceptableAnswers` list missing one of NESA's own accepted answers (2023 16(a)(i): 'sliding saw'), and **two stems that gave their own answers away** (2022 17(c), 2025 18(b)). ⚠️ **Mechanical triage ordered the queue and decided nothing — and it had to**: two of the six defects sat at the BOTTOM of the triage on overlap 1.00, because a stem that leaks its answer scores as perfect agreement. **Remaining: 346 of 369** — Maths Advanced 126, Standard 2 151, Multimedia 29, HMS 40; Standard 2 also still missing 40 `keywords` and Multimedia 4. Next is **Multimedia Stage 6b**. Gate 6 and a Definition-of-Done item for every future port (`docs/porting-playbook.md` §6). |
