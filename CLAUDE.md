...
│   ├── mathematics-standard-2.json    ← 318 MC + 151 written + 73 tips + studyNotes (1 topic so far — F1 Money Matters pilot, block-ordered content + 6 revision questions; 15 more categories not yet built)
...
| Study Mode & Exam Mode front page (HMS prototype + Maths pilot) | 🔶 HMS built 2026-07-26 (full — diagrams, comparison tables, 74 revision questions, free-preview gating). Maths started 2026-07-28 — 1 of 16 syllabus categories done (F1 Money Matters, original-wording notes since Maths has no standalone prose source to port from, unlike HMS), remaining 15 categories not yet built, no writingScaffolds yet (empty-state handles it). Not yet started for Multimedia/VET (was phased as P2/P3 — plan superseded now that Maths is in progress, see `docs/HISTORY.md`). HMS's 4 non-photo diagrams (SMART goals grid, tapering bar chart, inverted-U SVG, HOWSCSE grid) still deferred — need bespoke CSS ported, not just an image copy. |
